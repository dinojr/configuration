...
(defun jc-notmuch-file-to-group (file)
  (let ((group (directory-file-name (file-name-directory file))))
    ...
    ;;   )
    (cond
     ((string-match "Local" group) (replace-regexp-in-string "/home/wilk/email/Local" "" group))
     ((string-match "Offline" group) (replace-regexp-in-string ".*/Maildir/\\(Gmail\\|Free\\)/" "nnimap+\\1Offline:" group))
     ((string-match "\\(ENT\\|Academie\\)" group) (replace-regexp-in-string ".*/Maildir/\\(ENT\\|Academie\\)/" "nnimap+\\1:" group))
     )
    ))
