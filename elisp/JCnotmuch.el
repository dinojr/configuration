...
(defun jc-notmuch-file-to-group (file)
  (let ((group (directory-file-name (file-name-directory (directory-file-name (file-name-directory file))))))
    (cond
     ((string-match "Local" group) (setq group (replace-regexp-in-string "/home/wilk/email//Local/" "" group)))
     ((string-match "\\(Gmail\\|Free\\)" group) (setq group (replace-regexp-in-string ".*/Maildir/\\(Gmail\\|Free\\)/" "nnimap+\\1Offline:" group)))
     ((string-match "\\(Academie\\|ENT\\)" group) (setq group (replace-regexp-in-string ".*/Maildir/\\(Academie\\|ENT\\)/" "nnimap+\\1:" group)))
     )
